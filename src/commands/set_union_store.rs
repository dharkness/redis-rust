...
impl Apply for SetUnionStore {
    fn apply(&self, store: &mut Store, client: &mut Client, registry: &Registry) -> io::Result<()> {
        let union = match union(store, &self.from, usize::MAX) {
            Union::Set(members) => members,
            Union::SetRef(members) => members.clone(),
            Union::Empty => return client.write_empty_set(registry),
            Union::WrongType => return client.write_simple_error(WRONG_TYPE, registry),
        };
